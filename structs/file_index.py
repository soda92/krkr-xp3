...
from .constants import (
    XP3Signature,
    XP3FileIndexContinue,
    XP3FileIndexCompressed,
    Xp3FileIndexUncompressed,
)
...
class XP3FileIndex:
    def __init__(self, entries: list, buffer=None):
        ...
        self.path_index = {
            entry.file_path: index for index, entry in enumerate(entries)
        }
        self.buffer = buffer
    ...
    def to_bytes(self):
        ...
        if (
            compressed_size + 1 + 8 + 8 < uncompressed_size + 1 + 8
        ):  # Account for header overhead
            return (
                struct.pack(
                    "<BQQ", XP3FileIndexCompressed, compressed_size, uncompressed_size
                )
                + compressed_index
            )
        else:
            return (
                struct.pack("<BQ", Xp3FileIndexUncompressed, uncompressed_size)
                + uncompressed_index
            )
