...
class FolderReadAndWrite(unittest.TestCase):
    ...
    dummy_data = (("dummyfile1", b"dummydata1"), ("dummyfile2", b"dummydata2"))

    def test(self):
        with (
            tempfile.TemporaryDirectory() as datadir,
            tempfile.TemporaryDirectory() as xp3dir,
        ):
            ...
            with XP3(xp3_path, mode="r", silent=True) as xp3:
                ...
                xp3.extract(
                    os.path.join(xp3dir, "out")
                )  # Extract the archive to folder
                xp3.file_index.extract(
                    os.path.join(xp3dir, "data.xp3.index")
                )  # Dump file index
...
class MemoryReadAndWrite(unittest.TestCase):
    ...
    dummy_data = (
        ...
        (
            ...
            3000,
            False,
        ),  # compressed and uncompressed sizes should match, do not compress
    )
    ...
    def test(self):
        ...
        with XP3Reader(archive, silent=True) as xp3:
            for index, (filepath, data, timestamp, compressed) in enumerate(
                self.dummy_data
            ):
                file = xp3.open(index)
