...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="KiriKiri .XP3 archive repacking and extraction tool"
    )
    parser.add_argument(
        "-mode",
        "-m",
        choices=["e", "r", "extract", "repack"],
        default="e",
        help="Operation mode",
    )
    ...
    parser.add_argument(
        ...
        "-i",
        action="store_true",
        help="Dump the file index of an archive",
    )
    parser.add_argument(
        "-encryption",
        "-e",
        choices=encryption_parameters.keys(),
        default="none",
        help="Specify the encryption method",
    )
    parser.add_argument(
        "input", type=input_filepath, help="File to unpack or folder to pack"
    )
    parser.add_argument(
        "output", help="Output folder to unpack into or output file to pack into"
    )
